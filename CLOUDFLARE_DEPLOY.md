...
每次前端版本更新時，請同步提高 `BUILD_VERSION`，並更新 `public/version.json` 與 Service Worker 的 `CACHE_NAME`。目前版本為 `2026.08.21.9`。底部導覽使用棋盤墨黑、金色格線、紅色玩家標記與內嵌像素 SVG 圖示，不依賴外部圖片資產。Service Worker 對同源靜態檔採用 network-first；每次啟用新版本會刪除舊 cache，前端則在偵測到新版本時顯示更新提示。`/api/`、`/ws/` 與 `sw.js` 不會被 Service Worker 快取，因而避免遊戲狀態或認證流程被舊資料卡住。
