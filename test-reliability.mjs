...
assert.match(appSource,/purchaseConfirmationHTML/);
assert.match(appSource,/確定購買/);
assert.match(appSource,/class="team-dice-count"/);
...
assert.match(appSource,/teamPreview=mobileTeam&&!teamBoardExpanded\(\)/);
assert.match(appSource,/teamPreview\?Math\.min\(180,window\.innerHeight\*\.24\)/);
assert.match(appSource,/team-tab-\$\{App\.tab\}/);
assert.match(stylesSource,/body\.role-team \.team-persistent-layout>\.game-primary\{order:1;position:sticky/);
assert.match(stylesSource,/body\.role-team \.team-persistent-layout>\.team-tab-panel\{order:2/);
...
assert.match(appSource,/function dicePhysicsStyle/);
assert.match(appSource,/physics:true/);
assert.match(stylesSource,/@keyframes dicePhysicsArc/);
assert.match(stylesSource,/@keyframes dicePhysicsSpin/);
assert.match(stylesSource,/--physics-die-size/);
assert.match(stylesSource,/稅務稽查/);
assert.match(appSource,/function hostBattleActor/);
assert.match(appSource,/id="jailAcceptPenalty"/);
assert.match(appSource,/挑戰主持人/);
assert.match(stylesSource,/\.jail-encounter/);
...
const legacyJail=G.freshState('LEGACY-JAIL',2);
legacyJail.teams[0].jail=1;
legacyJail.teams[0].jailedThisTurn=true;
normalizeGameState(legacyJail);
assert.equal(legacyJail.teams[0].jail,0);
assert.equal(legacyJail.teams[0].jailedThisTurn,false);
...
const jailFlow=G.freshState('JAIL-BATTLE-FLOW',2),jailTile=G.TRACK.findIndex(tile=>tile[0]==='jail');
jailFlow.phase='roll';jailFlow.round=2;jailFlow.teams[0].baseIdx=G.BASE_IDX[0];jailFlow.teams[0].level=2;jailFlow.teams[0].pos=jailTile;
G.landEffect(jailFlow,0,[]);
const jailBattleQuota=jailFlow.teams[0].battles;
assert.equal(jailFlow.pendingBattle?.kind,'jail');
assert.equal(configRoom.applyAction(jailFlow,{role:'team',teamId:0},'resolveLanding',{choice:'battle'}),undefined);
assert.equal(jailFlow.teams[0].battles,jailBattleQuota-1);
assert.equal(configRoom.applyAction(jailFlow,{role:'host',teamId:null},'resolveBattle',{outcome:'attacker'}),undefined);
assert.equal(jailFlow.teams[0].level,2);
assert.equal(jailFlow.teams[0].sold,false);
assert.equal(jailFlow.lastJailBattle?.outcome,'escaped');
...
assert.match(configRoom.applyAction(rollPermissionState,{role:'host',teamId:null},'allowRoll',{teamId:0,diceCount:31}).error,/1～30/);
assert.equal(configRoom.applyAction(rollPermissionState,{role:'host',teamId:null},'allowRoll',{teamId:0,diceCount:30}),undefined);
...
assert.equal(rollPermissionState.teams[0].lastDice.length,30);
...
assert.equal(configRoom.applyAction(rollPermissionState,{role:'host',teamId:null},'allowRoll',{teamId:1,diceCount:11}),undefined);
assert.equal(configRoom.applyAction(rollPermissionState,{role:'team',teamId:1},'roll',{}),undefined);
assert.equal(rollPermissionState.teams[1].lastDice.length,11);
assert.deepEqual(rollPermissionState.rollDiceCounts,{0:30,1:11});
G.nextPhase(rollPermissionState);
assert.deepEqual(rollPermissionState.rollDiceCounts,{});
...
const lootReceiptSockets=[],lootReceiptRoom=new GameRoom({blockConcurrencyWhile:fn=>fn(),storage:{},getWebSockets:()=>lootReceiptSockets},{});
lootReceiptRoom.loaded=true;lootReceiptRoom.meta={id:'LOOT-RECEIPT',name:'實體兌換收據測試',teamCount:2,hostTokenHash:'unused'};lootReceiptRoom.state=normalizeGameState(G.freshState('LOOT-RECEIPT',2));lootReceiptRoom.state.teams[0].items.g3=2;lootReceiptRoom.commit=async next=>{lootReceiptRoom.state=next;};
const lootHostSocket=pendingSocket();lootHostSocket.serializeAttachment({role:'host',teamId:null});lootReceiptSockets.push(lootHostSocket);
const zeroLootCash=lootReceiptRoom.state.teams[0].cash;
await lootReceiptRoom.webSocketMessage(lootHostSocket,JSON.stringify({type:'action',action:'redeemPhysical',payload:{teamId:0,itemIndex:3,reward:0},actionId:'loot-zero-once'}));
const zeroLootReceipt=lootReceiptRoom.state.receipts.find(receipt=>receipt.action==='redeemPhysical');
assert.equal(lootReceiptRoom.state.teams[0].cash,zeroLootCash);assert.equal(lootReceiptRoom.state.teams[0].items.g3,1);assert.equal(zeroLootReceipt.cashDelta,0);assert.equal(zeroLootReceipt.displayCash,true);assert.equal(zeroLootReceipt.category,'physical_redeem');assert.match(zeroLootReceipt.reason,/未中獎/);
await lootReceiptRoom.webSocketMessage(lootHostSocket,JSON.stringify({type:'action',action:'redeemPhysical',payload:{teamId:0,itemIndex:3,reward:1500},actionId:'loot-win-once'}));
assert.equal(lootReceiptRoom.state.teams[0].cash,zeroLootCash+1500);assert.equal(lootReceiptRoom.state.teams[0].items.g3,0);assert.equal(lootReceiptRoom.state.receipts.filter(receipt=>receipt.action==='redeemPhysical').length,2);
await lootReceiptRoom.webSocketMessage(lootHostSocket,JSON.stringify({type:'action',action:'redeemPhysical',payload:{teamId:0,itemIndex:3,reward:1500},actionId:'loot-win-once'}));
assert.equal(lootReceiptRoom.state.teams[0].cash,zeroLootCash+1500);assert.equal(lootReceiptRoom.state.receipts.filter(receipt=>receipt.action==='redeemPhysical').length,2);
await cardReceiptRoom.webSocketMessage(cardHostSocket,JSON.stringify({type:'action',action:'resolveCard',payload:{outcome:'success'},actionId:'card-result-once'}));
assert.equal(cardReceiptRoom.state.teams[0].cash,cardCashBefore+cardSuccess);assert.equal(cardReceiptRoom.state.receipts.filter(receipt=>receipt.action==='resolveCard').length,1);
...
secrets.teams[0].itemPurchases={g3:1};secrets.teams[0].physicalPurchaseRound=2;
