const BUILD_VERSION = '2026.09.04.64';
import { G } from './game-core.js?v=2026.09.04.64';
import { PHASE_FX, ATTACK_FX, CEREMONY_STEPS, ceremonyStep, SoundFX, isSoundEnabled, toggleSound, classifyEvent, movementPath, movementStepDelay, presentationTier, isPresentationTaskRelevant, isPurchaseReceipt, renderPawnSprite, renderTileGarrison, PAWN_ARCHETYPES, pawnFacingForStep, battlePresentationTransition, landingReactionForTile, attackCharacterTargets, stagePresentationFor } from './game-fx.js?v=2026.09.04.64';
...
function executeBattleDuelFx(task,done){
  const battle=task.battle,teams=task.teams||App.state?.teams||[],attacker=teams[battle?.attackerId],defender=teams[battle?.defenderId];
...
function pawnVisualStatus(teamId,pos,team){
  ...
  return {
    ...
    status:{
      ...
      isJailed:Number(targetTeam?.jail||0)>0||pos===42,
...
function battleEncounterHTML(){
  ...
  const attacker=S.teams?.[pending.attackerId],defender=S.teams?.[pending.defenderId],battles=Number(attacker?.battles||0),isCard=pending.kind==='card';
  if(isCard){
...
function battleDuelHTML(){
  ...
  const isCard=fx.battle.kind==='card',card=isCard?G.cardById(fx.battle.cardType,fx.battle.cardId):null;
  return `<div class="battle-duel-overlay" aria-live="assertive" style="--attacker:${fx.attacker.color};--defender:${fx.defender.color}"><div class="battle-duel-speedlines" aria-hidden="true"></div><div class="battle-duel-kicker">${isCard?'CARD STAKE BATTLE':'BATTLE ENGAGED'} // HOST DECISION</div><div class="battle-duel-stage"><section class="battle-fighter attacker"><div class="battle-hero-shell">${battlePawnHTML(fx.attacker,{direction:'right',pose:'battle',extraClass:'battle-hero battle-hero-attacker',scale:3.7})}</div><small>CHALLENGER</small><b>${esc(fx.attacker.name)}</b></section><div class="battle-clash-mark"><span>⚔</span><b>VS</b></div><section class="battle-fighter defender"><div class="battle-hero-shell">${battlePawnHTML(fx.defender,{direction:'left',pose:'battle',extraClass:'battle-hero battle-hero-defender',scale:3.7})}</div><small>${isCard?'CARD TARGET':'BASE GUARDIAN'}</small><b>${esc(fx.defender.name)}</b></section></div><div class="battle-duel-note">${isCard?`${cardTypeLabel(fx.battle.cardType)}卡「${esc(card?.name||'挑戰')}」執行權爭奪中`:`過夜費 ${G.money(fx.battle.amount)} 凍結中`} · 等待主持人裁決</div></div>`;
}
function battleResultHTML(){
  const fx=App.fx.battleResult;if(!fx)return '';
  const attackerWon=fx.outcome==='attacker',isCard=fx.battle.kind==='card',title=isCard?(attackerWon?'卡片轉移成功！':'守方反制成功！'):(attackerWon?'挑戰成功！':'基地守住！'),subtitle=isCard?(attackerWon?`卡片改由 ${fx.defender.name} 執行`:`卡片仍由 ${fx.attacker.name} 執行`):(attackerWon?`過夜費 ${G.money(fx.battle.amount)} 全額免除`:`支付 ${Number(App.state?.settings?.battleLossMultiplier)||150}% BATTLE 懲罰`);
...
function teamControls(){
  ...
  if(S.phase==='roll'){
    ...
    if(myPending&&pending.status==='awaiting_choice'){
      ...
      h+=pending.kind==='card'?`<div class="battle-choice"><div class="battle-kicker">CARD ENCOUNTER</div><h3>${cardTypeLabel(pending.cardType)}卡「${esc(card?.name||'挑戰')}」</h3><p>移動演出完成後可選擇本隊執行，或消耗一次 BATTLE 指定其他隊伍。</p></div>`:`<div class="battle-choice"><div class="battle-kicker">BASE ENCOUNTER</div><h3>抵達 ${esc(defender?.name||'對手')} 的基地</h3><p>過夜費 <b>${G.money(pending.amount)}</b> 尚未扣款；移動演出完成後會出現付款或 BATTLE 選擇視窗。</p></div>`;
    }else if(myPending&&pending.status==='awaiting_host'){
      h+=`<div class="battle-choice waiting"><div class="battle-kicker">BATTLE IN PROGRESS</div><h3>⚔️ 等待主持人裁決</h3><p>${pending.kind==='card'?'勝方將決定這張卡片最後由哪一隊執行。':`過夜費 ${G.money(pending.amount)} 仍未扣款；攻方勝免付，守方勝支付 ${Number(S.settings.battleLossMultiplier)||150}% 懲罰。`}</p></div>`;
...
function hostPanel(){
  ...
  if(section==='flow'){
    ...
    if(S.phase==='roll'){
      ...
      h+=`<div class="host-turn-status ${active?'active':''}">${active?`現在輪到 <b>${esc(active.name)}</b> 操作`:'點選下方隊伍開放擲骰'}</div><div class="host-roll-grid ${App.hostTestMode?'test-mode':''}">${S.teams.map((t,i)=>{
        ...
        const dicePicker=`<label class="host-team-dice-picker"><span>骰子</span><select class="team-dice-count" data-i="${i}" ${t.rolled||isJailed||S.pendingBattle||S.pendingCard?'disabled':''}>${[1,2,3,4,5].map(n=>`<option value="${n}" ${diceCount===n?'selected':''}>${n} 顆</option>`).join('')}</select></label>`;
        if(App.hostTestMode){
          return `<div class="host-test-team-card ${t.rolled?'completed':''} ${isJailed?'jailed':''}"><div class="host-test-team-info"><span class="sw" style="background:${t.color}">${i+1}</span><div class="host-test-team-name"><b>${esc(t.name)}</b><small>${status}</small></div></div><div class="host-test-team-actions"><button class="btn xs green test-roll-btn" data-i="${i}" ${t.rolled||isJailed||S.pendingBattle||S.pendingCard?'disabled':''} title="主持人直接替該隊擲出 ${testSteps} 步">⚡ 代擲 ${testSteps}</button><button class="btn xs ${isPreset?'gold':'outline'} test-preset-btn" data-i="${i}" ${t.rolled||isJailed||S.pendingBattle||S.pendingCard?'disabled':''} title="預設該隊下次擲骰為 ${testSteps} 步">${isPreset?`✓ 預設 ${S.presetRolls[i]}`:`🎯 預設 ${testSteps}`}</button>${isPreset?`<button class="btn xs dark test-clear-preset-btn" data-i="${i}" title="清除預設">×</button>`:''}${dicePicker}<button class="btn xs ${S.activeTeamId===i?'green':'outline'} allow-roll" data-i="${i}" ${t.rolled||isJailed||S.pendingBattle||S.pendingCard?'disabled':''} title="允許隊輔使用 ${diceCount} 顆骰子">${S.activeTeamId===i?'開放中':'允許'}</button></div></div>`;
        }
        return `<div class="host-roll-team-card ${S.activeTeamId===i?'active':''} ${t.rolled?'completed':''} ${isJailed?'jailed':''}"><div class="host-test-team-info"><span class="sw" style="background:${t.color}">${i+1}</span><div class="host-test-team-name"><b>${esc(t.name)}</b><small>${status}</small></div></div><div class="host-roll-team-actions">${dicePicker}<button class="btn xs ${S.activeTeamId===i?'green':'outline'} allow-roll" data-i="${i}" ${t.rolled||isJailed||S.pendingBattle||S.pendingCard?'disabled':''}>${S.activeTeamId===i?'已開放':'允許擲骰'}</button></div></div>`;
      }).join('')}</div></section>`;
    }
    if(S.pendingBattle){const p=S.pendingBattle,attacker=S.teams[p.attackerId],defender=S.teams[p.defenderId],isCard=p.kind==='card',card=isCard?G.cardById(p.cardType,p.cardId):null;h+=`<div class="host-battle-panel ${isCard?'card-stake':''}"><div class="sub">⚔️ ${isCard?'卡片爭奪戰':'基地 BATTLE'}待裁決</div><p><b>${esc(attacker?.name||'攻方')}</b>${defender?` 挑戰 <b>${esc(defender.name)}</b>`:''}${isCard?`，賭注是${cardTypeLabel(p.cardType)}卡「${esc(card?.name||'挑戰')}」的執行權。`:`，原過夜費 ${G.money(p.amount)}；攻方落敗支付 ${Number(S.settings.battleLossMultiplier)||150}%。`}</p>${p.status==='awaiting_host'?`<div class="battle-actions"><button class="btn sm green battle-result" data-outcome="attacker">攻方勝 · ${isCard?'對手執行':'免付'}</button><button class="btn sm dark battle-result" data-outcome="defender">守方勝 · ${isCard?'原隊執行':`${Number(S.settings.battleLossMultiplier)||150}% 收費`}</button></div>`:'<div class="note">等待攻方選擇直接處理或發動 BATTLE。</div>'}</div>`;}
  ...
  if(section==='teams'){
    h+=`<div class="host-section-intro"><b>隊伍與裝置</b><span>在隊伍清單直接修改隊名、查看連線健康並即時調整資源。</span></div><section class="host-work-card"><div class="host-work-title"><span>隊伍管理與即時調整</span><small>${hostOnlineTeamCount()} 隊健康在線 ｜ 輸入正數增加、負數扣除</small></div><div class="host-unified-team-list">${S.teams.map((t,i)=>{const editing=App.editingTeamName===i,draft=App.hostDrafts[`name:${i}`]??t.name,presence=hostPresenceStatus(i);return `<div class="host-team-row ${editing?'editing':''}"><div class="host-team-head"><span class="sw" style="background:${t.color}">${i+1}</span><div class="host-team-main">${editing?`<div class="team-edit-input-row"><input class="team-name-edit" data-i="${i}" value="${esc(draft)}" maxlength="30" aria-label="修改第 ${i+1} 組隊名"><button type="button" class="team-name-save" data-i="${i}" title="儲存隊名">✓</button><button type="button" class="team-name-cancel" data-i="${i}" title="取消修改">×</button></div>`:`<div class="team-name-display"><b>${esc(t.name)}</b><button type="button" class="team-name-edit-button" data-i="${i}" title="修改隊名">✎</button><span class="host-team-status connection-${presence.state}" data-presence-team="${i}">${esc(presence.label)}</span></div>`}<div class="host-team-wealth"><span>💰 ${G.money(t.cash)}</span><span>✨ ${t.pts} 點</span>${t.jail>0?`<span class="host-jail-tag">⛓️ 監獄 (${t.jail})</span>`:''}</div></div>${t.joined&&!editing?`<button class="btn xs dark kick" data-i="${i}">踢出</button>`:''}</div><div class="host-team-adjustments"><div class="ain"><input class="cash" data-i="${i}" type="number" inputmode="numeric" placeholder="現金 (+/-)"><button class="btn xs gold csgo" data-i="${i}">套用</button></div><div class="ain"><input class="pts" data-i="${i}" type="number" inputmode="numeric" placeholder="點數 (+/-)"><button class="btn xs blue ptgo" data-i="${i}">套用</button></div></div></div>`;}).join('')}</div></section>`;
...
function bindGame(){
  ...
  if(App.role==='team'&&App.teamId!==null){
    const me=S.teams[App.teamId];bindDiceGesture();bind('bReroll',()=>ask('使用重骰卡？','會消耗一張重骰卡，並立即重新取得本組擲骰權限。',()=>send('reroll')));bind('battlePayNow',()=>send('resolveLanding',{choice:'pay'},{preserveView:true}));bind('battleAcceptCard',()=>send('resolveLanding',{choice:'accept'},{preserveView:true}));bind('battleFightNow',()=>{const target=document.querySelector('#cardBattleTarget');send('resolveLanding',{choice:'battle',...(target?{targetTeamId:Number(target.value)}:{})},{preserveView:true});});bind('bUp',()=>send('upgrade'));bind('bSell',()=>send('sell'));bind('bBuyBack',()=>send('buyBack'));
  ...
  if(App.role==='host'){
    ...
    document.querySelectorAll('.battle-result').forEach(b=>b.onclick=()=>{const outcome=b.dataset.outcome,p=S.pendingBattle,isCard=p?.kind==='card',label=isCard?(outcome==='attacker'?'攻方獲勝，卡片交由守方執行':'守方獲勝，卡片仍由原攻方執行'):(outcome==='attacker'?'攻方獲勝並免付過夜費':`守方獲勝並收取 ${Number(S.settings.battleLossMultiplier)||150}% 過夜費懲罰`);ask('確認 BATTLE 裁決？',label,()=>send('resolveBattle',{outcome}));});
