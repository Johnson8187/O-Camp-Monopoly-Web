...
assert.match(configRoom.applyAction(rollPermissionState,{role:'host',teamId:null},'allowRoll',{teamId:0,diceCount:11}).error,/1～10/);
assert.equal(configRoom.applyAction(rollPermissionState,{role:'host',teamId:null},'allowRoll',{teamId:0,diceCount:10}),undefined);
...
assert.equal(rollPermissionState.teams[0].lastDice.length,10);
...
assert.deepEqual(rollPermissionState.rollDiceCounts,{0:10,1:4});
