...
function statusOf(state){ return state.phase==='ended'?'ended':state.phase==='setup'?'lobby':'running'; }
  ...
  async fetch(request, env){
    ...
    try{
      if(url.pathname==='/api/lobby' && request.method==='GET'){
        const rows=await env.DB.prepare('SELECT id,name,status,team_count,updated_at,state_json FROM games WHERE status IN (\'lobby\',\'running\') ORDER BY updated_at DESC LIMIT 50').all();
...
const HOST_ACTIONS=new Set(['assignBases','startGame','nextPhase','endGame','setMarket','unlock','adjustCash','adjustPts','renameTeams','setConfig']);
const TEAM_ACTIONS=new Set(['roll','reroll','battle','attack','gamble','buff','upgrade','sell','buyBack']);

export class GameRoom {
  constructor(ctx,env){ this.ctx=ctx;this.env=env;this.loaded=false;this.state=null;this.meta=null;this.gameId=null; }
  async load(){
  ...
  async webSocketMessage(ws,message){
    ...
    if(actor.role==='pending'){
      ...
      if(role==='team'&&this.state.teams[teamId]&&!this.state.teams[teamId].joined){ const next=G.clone(this.state);next.teams[teamId].joined=true;next.log.unshift(`${next.teams[teamId].name} 已加入活動`);await this.commit(next,actor,'teamJoin',{}); }
