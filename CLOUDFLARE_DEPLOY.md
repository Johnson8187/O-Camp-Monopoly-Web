...
D1 schema 已套用到遠端資料庫，包含 `games`、`game_events` 及必要索引。`wrangler.toml` 已經寫入實際 D1 UUID、Durable Object migration、SPA fallback 與密碼雜湊變數，因此直接開啟 `/team` 或 `/admin` 也會回傳同一個前端入口並進入對應密碼閘門。
...
假設 Worker 網址為 `https://preview.example.workers.dev`，首頁 `/` 預設只作觀戰入口。畫面底部導覽列提供三個角色入口：觀戰、隊輔與控制台。

| 導覽入口 | 使用者 | 用途 | 密碼 |
|---|---|---|---|
| `/` 或觀戰 | 隊員／觀眾 | 顯示唯一一場活動並進入唯讀觀戰 | 不需密碼 |
| `/team` 或隊輔導覽 | 隊輔 | 選擇隊伍、輸入主持人提供的隊伍 PIN | `iii` |
| `/admin` 或控制台導覽 | 主持人 | 建立、開始、暫停、恢復、結束活動與管理隊輔 | `aaa` |

`/team` 與 `/admin` 是保留的深層連結，不代表公開首頁會顯示三個獨立的參加方式區塊。前端登入只是使用者體驗的一部分；`/api/auth`、建立活動、WebSocket `hello` 與關閉活動 API 都會在 Worker 端再次驗證。密碼的 SHA-256 雜湊位於 `wrangler.toml` 的 `[vars]`，不要在程式碼或公開訊息中新增明文密碼。
...
部署完成後，請以一般瀏覽器分別確認首頁、隊輔深層連結與主持人深層連結都能載入：
...
主持人先開啟首頁底部的「控制台」，輸入密碼 `aaa`，再輸入活動名稱與隊伍數量並建立活動。建立後，控制台會顯示各隊一次性 PIN，請透過私下方式交給對應隊輔。隊輔開啟底部的「隊輔」入口，輸入密碼 `iii`，選擇自己的隊伍並輸入 PIN；隊員與觀眾直接進入首頁後選擇「進入觀戰」。

主持人可以重新抽籤、開始遊戲、暫停或恢復活動、切換階段、調整隊伍名稱與資源、公布股市、解封關卡、查看 D1 歷史紀錄，以及在「隊輔連線」區塊踢除目前在線的隊輔。被踢的隊輔 WebSocket 會立即關閉，該隊回到未連線狀態；隊輔可以重新輸入正確密碼與 PIN 加入。
...
每次前端版本更新時，請同步提高 `BUILD_VERSION`，並更新 `public/version.json` 與 Service Worker 的 `CACHE_NAME`。目前版本為 `2026.08.21.13`。底部導覽使用棋盤墨黑、金色格線、紅色玩家標記與內嵌像素 SVG 圖示，不依賴外部圖片資產。Service Worker 對同源靜態檔採用 network-first；每次啟用新版本會刪除舊 cache，前端則在偵測到新版本時顯示更新提示。`/api/`、`/ws/` 與 `sw.js` 不會被 Service Worker 快取，因而避免遊戲狀態或認證流程被舊資料卡住。
...
本地端到端測試會啟動隔離的 Worker 與 D1 狀態，驗證首頁入口、錯誤密碼拒絕、角色登入、建立活動、WebSocket 觀戰與隊輔連線、踢除隊輔、D1 事件紀錄，以及沒有主持人 WebSocket 時仍能由 API 關閉活動：

```bash
cd /home/ubuntu/preview_repo
rm -rf /tmp/preview-e2e-state
npx wrangler@latest dev --local --port 8787 --persist-to /tmp/preview-e2e-state
# 另一個終端機
node test-single-activity.mjs
```
