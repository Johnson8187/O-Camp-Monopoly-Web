...
- **操作面板動態標註**：
  ...
  - 🟡 **賣出基地**：依當前回合房市倍率即時計算收益（例如 `賣出基地（+$500）`）。
  - 🔵 **買回基地**：清楚顯示買回成本（例如 `買回基地（$500）`），當回合賣出自動鎖定下回合開放。
- **監獄狀態防護**：入獄組別於擲骰階段自動鎖定並顯示服刑中提示，不觸發多餘移動動畫。

### 5. 🏆 最終結算與榮譽頒獎典禮
- **獨立結算流程**：主持人主控台專屬金色 `🏆 進行最終結算` 按鈕，與結束活動徹底隔離，杜絕操作失誤。
- **3D 立體頒獎台 (Podium)**：結算時全場同步播放 8-bit 勝利號角，自動切換至專屬頒獎分頁，展示 🥇 總冠軍、🥈 亞軍、🥉 季軍立體階梯與總資產戰報。
- **營隊榮譽榜與全隊總表**：評選出「💰 現金富豪」、「🏰 地產大亨」、「✨ 諂媚之王」三項特別大獎，並列出全隊現金、房產市值與點數完整明細。
- **安全封存機制**：結束活動設有專屬「⚠️ 活動封存危險區」與二次確認，確認頒獎結束後再封存資料庫。
